...
DEFAULT_INPUT_JSON = 'with_0_1_2.json'
DEFAULT_OUTPUT_XML = 'cvat_output_yolo.xml'
WIDTH = 1920
HEIGHT = 1080
CLASS_NAMES = {0: 'person', 1: 'bicycle', 2: 'car'}


def convert(input_json, output_xml, task_name=None, width=WIDTH, height=HEIGHT, class_names=CLASS_NAMES):
    ...
    for frame_data in data:
        ...
        cls_names = frame_data.get('class_names', [])
        ids = frame_data.get('ids', [])
        seen_track_frames = set()

        for i in range(len(boxes)):
            x1, y1, x2, y2 = boxes[i]
            score = scores[i] if i < len(scores) else 0.0

            if i < len(cls_names) and cls_names[i] in NAME_TO_CLS:
                class_id = NAME_TO_CLS[cls_names[i]]
            else:
    ...
    if task_name is None:
        task_name = os.path.splitext(os.path.basename(input_json))[0]
    ...
    print(f"Writing XML: {output_xml}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Convert YOLO JSON to CVAT XML.')
    parser.add_argument('inputs', nargs='*', help='Input YOLO JSON file(s). If omitted, uses the default file.')
    parser.add_argument('-o', '--output', help='Output XML path (only valid with a single input).')
    parser.add_argument('--width', type=int, default=WIDTH)
    parser.add_argument('--height', type=int, default=HEIGHT)
    args = parser.parse_args()

    if not args.inputs:
        convert(DEFAULT_INPUT_JSON, DEFAULT_OUTPUT_XML, width=args.width, height=args.height)
    else:
        if args.output and len(args.inputs) > 1:
            parser.error('--output can only be used with a single input file.')
        for input_json in args.inputs:
            if args.output:
                output_xml = args.output
            else:
                base = os.path.splitext(input_json)[0]
                output_xml = base + '_cvat.xml'
            convert(input_json, output_xml, width=args.width, height=args.height)
